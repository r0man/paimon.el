    ...
    (save-window-excursion
      (let* ((_profile (closql-insert db (paimon-profile :default 1)))
        ...
        (with-current-buffer (get-buffer (paimon-search-job-buffer-name job))
          ...
          (let ((result (paimon-search-result-under-point)))
            (should (cl-typep result paimon-search-result))
            (should (equal 0 (oref (paimon-search-result-under-point) offset))))
          (paimon-search-results-next-line)
          (let ((result (paimon-search-result-under-point)))
            (should (cl-typep result paimon-search-result))
            (should (equal 1 (oref (paimon-search-result-under-point) offset)))))))))
